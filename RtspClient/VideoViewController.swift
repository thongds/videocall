...
class VideoViewController: UIViewController, WZStatusCallback, WZVideoSink, WZAudioSink {
    ...
    public func startBroadcast(broadCast : String){
        print(broadCast)
        goCoderConfig.hostAddress = "103.69.195.137"
        goCoderConfig.portNumber = 1935
        goCoderConfig.applicationName = "videochat"
        goCoderConfig.streamName = broadCast
        //goCoderConfig.streamName = broadCast
        ...
        if !goCoderRegistrationChecked {
            ...
            if let goCoderLicensingError = WowzaGoCoder.registerLicenseKey(SDKSampleAppLicenseKey) {
                ...
                if let goCoder = WowzaGoCoder.sharedInstance() {
                    ...
                    self.goCoder?.cameraView?.frame = CGRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height/2)
